...
const CarouselDots: React.FC = () => {
  ...
  // Calculate the number of dots based on chunk size
  const numberOfDots = Math.ceil(scrollSnaps.length / chunkSize);

  // Calculate the start index for each dot
  const getDotIndexRange = (dotIndex: number) => {
    const startIndex = dotIndex * chunkSize;
